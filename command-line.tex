...
\begin{lstlisting}
[gestionserv] (688) ~ % find /etc -name  "*" | xargs stat --printf="%z %n\n" | sort
...
\begin{lstlisting}
[gestionserv] (689) ~ % find /etc -name  "*" -print0 | xargs -0  stat --printf="%z %n\n" | sort
...
...
\begin{lstlisting}
[gestionserv] (694) ~ % for d in /bin /sbin /lib /lib32 /usr/bin /usr/sbin /usr/lib /usr/lib32; do
...
\begin{lstlisting}
[gestionserv] (695) ~ % for d in /bin /sbin /lib /lib32 /usr/bin /usr/sbin /usr/lib /usr/lib32; do
find $d -name  "*" -print0 | xargs -0  stat --printf="%z %n\n" | egrep "^2013-11-0[1-7]"
done | sort
[gestionserv] (696) ~ %
\end{lstlisting}
...
\begin{lstlisting}
[gestionserv] (695) ~ % for d in /bin /sbin /lib /lib32 /usr/bin /usr/sbin; do
find $d -name  "*" -print0 | xargs -0  stat --printf="%z %n\n" | egrep "^2013"
done | sort
...
[gestionserv] (696) ~ %
...
Les guerres de religions qui opposent parfois violemment les mondes \BSD et \linux, les supporters de \code{Vi} et \code{Emacs}, ... finissent par être absorbées avec le temps et seuls quelques irréductibles les raniment, souvent plus pour s'exposer aux yeux (blasés maintenant) du petit monde concerné. Seule reste l'opposition farouche entre tenants du libre et leurs opposants.
